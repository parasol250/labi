/*В данной работе необходимо использовать один из шаблонов проектирования Strategy, реализовать наследование, полиморфизм и парадигму интерфейсных классов.
В работе для обработки ошибок должны быть реализованы исключения (часть проверок можно сделать классическим способом).
Сначала необходимо создать диаграмму классов, а потом уже начинать реализацию.
Вариант №2
Предметная область: Вокзал. Класс «вокзал» имеет список тарифов на различные направления.
На некоторые тарифы может быть предоставлена скидка, заданная в процентах.
В классе «вокзал» реализовать методы добавления нового тарифа со скидкой и без скидки, поиск направления с минимальной стоимостью.
*/
#include <iostream>
#include <map>
#include <windows.h>
